...
public class LongestAbsoluteFilePath {
    public int lengthLongestPath(String input) {
        ...
        // The stack array is used to maintain the cumulative length of paths at different directory levels (or depths)

        int maxLen = 0;
        for (String s : paths) {
            int lev = s.lastIndexOf("\t") + 1;
            // Prev levels sum + this level's char counts + 1 for \
            stack[lev + 1] = stack[lev] + (s.length() - lev) + 1;
            if (s.contains(".")) {
                // at the end remove the extra \ contribution
